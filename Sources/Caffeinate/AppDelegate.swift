...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Default break duration (seconds) — 5 minutes.
    private var breakDurationSeconds: Int = 5 * 60
    ...
    /// When true, break reminders are suspended indefinitely. Toggle off to resume.
    private var unleashed: Bool = false
    private var unleashItem: NSMenuItem!
    ...
    private func buildStatusItem() {
        ...
        unleashItem = NSMenuItem(title: "Unleash", action: #selector(toggleUnleash), keyEquivalent: "u")
        unleashItem.target = self
        unleashItem.state = unleashed ? .on : .off
        menu.addItem(unleashItem)
        ...
        let about = NSMenuItem(title: "About Caffeinate", action: #selector(showAbout), keyEquivalent: "")
        about.target = self
        menu.addItem(about)
    ...
    private func refresh(force: Bool) {
        ...
        if breakInProgress {
            ...
            timeToBreakItem.title = "Unleashed — no breaks"
        } else if isSnoozed() {
    ...
    @objc private func toggleUnleash() {
        unleashed.toggle()
        unleashItem.state = unleashed ? .on : .off
        if unleashed { activeSeconds = 0 }
        savePrefs()
        refresh(force: true)
    }
    ...
    private func loadPrefs() {
        ...
        unleashed = d.bool(forKey: "unleashed")
    }
    ...
    private func savePrefs() {
        ...
        d.set(unleashed, forKey: "unleashed")
    }
